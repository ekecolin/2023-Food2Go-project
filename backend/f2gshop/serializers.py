...
class ProductSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = Product
        fields = ['id', 'name', 'ingredients', 'nutritional_information', 'macros', 'allergens', 'recipe', 'price', 'image']

class OrderSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = Order
        fields = ['id', 'name', 'sname', 'date_ordered', 'basket_id', 'user_id', 'total_price', 'shipping_address', 'city', 'country', 'postcode']

class BasketItemSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = BasketItem
        fields = ['id', 'basket_id', 'product_name', 'quantity', 'price']
...
class CheckoutSerializer(serializers.ModelSerializer):
    class Meta:
        model = Order
        fields = ['basket_id', 'shipping_address']

    def create(self, validated_data):
        request = self.context.get('request', None)
        current_user = request.user
        basket_id = validated_data['basket_id']
        shp = validated_data['shipping_address']
        ...
        # create a new order 
        order = Order.objects.create(basket_id = basket_id, user_id = current_user, shipping_address =shp, total_price = total )
